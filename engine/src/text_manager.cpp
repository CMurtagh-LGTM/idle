...
gsl::owner<CuteText*> TextManager::new_text(std::string_view new_text, Cute::v2 new_position) { return text.new_ptr(new_text, new_position); }
...
Cute::v2 CuteText::get_min_size() const {
  return Cute::text_size(text.c_str());
}
